...
function HolographicText({ text }: { text: string }) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative z-10 select-none mix-blend-screen"
    >
      <h1
        ref={redRef}
        className="absolute inset-0 text-center text-red-500 opacity-80 blur-[1px]"
      >
        {text}
      </h1>
      <h1
        ref={blueRef}
        className="absolute inset-0 text-center text-cyan-500 opacity-80 blur-[1px]"
      >
        {text}
      </h1>
      <h1
        ref={whiteRef}
        className="relative z-10 text-center text-white mix-blend-screen"
      >
        {text}
      </h1>
    </div>
        ...
        <div className="w-full overflow-hidden whitespace-nowrap mask-gradient">
          <div className="inline-block animate-marquee">
            {[
              ...
              <span
                key={i}
                className="inline-block mx-6 text-sm font-bold tracking-wider text-blue-100/80"
              >
                {tech}
              </span>
            ))}
...
export default function HeroSection() {
  // If we've already shown it in this session, skip it.
  // We use sessionStorage to persist across page reloads/hard refreshes.
  const [isLoading, setIsLoading] = useState(() => {
    if (typeof window !== 'undefined') {
      return !sessionStorage.getItem('__HAS_SHOWN_LOADING__');
    }
    return true;
  });
  ...
  const handleLoadComplete = () => {
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('__HAS_SHOWN_LOADING__', 'true');
    }
    setIsLoading(false);
